...
class QuanLyCuaHangController extends Controller
{
    ...
    public function manage_oder(){
        ...
        $oder = DB::table('hoa_don')
                ...
                ->where('id_nb',$id_nd)
                ->select('trang_thai_don_hang.*','nguoi_dung.*','hoa_don.*')
                ->get();
    ...
    public function oder_detail(Request $request,$id){
        ...
        ->where('id_hd',$id)
        ->get();
        return view('client.quanly-cuahang.donhang.chitiet-donhang',compact('danhsach','khachhang','ttvc','mgg'));
    }

    public function accepOder($id){
        $update = DB::table('hoa_don')->where('id',$id)->update(
            [
                'id_tt'=>3
            ]
        );
        return \redirect()->route('manage_oder.index');
    }
